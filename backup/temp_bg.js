class BackgroundService {
    ...
    init() {
        ...
        chrome.alarms.onAlarm.addListener((alarm) => {
            ...
            if (alarm.name === 'dailyReminder') {
                ...
                this.keepAlive();
            } else if (alarm.name === 'testNotification') {
    ...
    sendTestNotification() {
        try {
            chrome.notifications.create('testNotification', {
                ...
                message: 'Your notifications are working perfectly! Memento Mori is active.'
    ...
    async sendDailyNotification() {
        ...
        let baseMessage;
        if (daysLeft < 0) {
            baseMessage = "You're living on borrowed time... Every second is a gift!";
        } else {
        ...
        try {
            ...
            console.log('Daily notification sent:', message);
        } catch (error) {
    ...
    async sendDeadlineNotification(goalIndex) {
        ...
        if (goal && goal.deadline && !goal.completed) {
            ...
            let message;
            if (daysLeft === 0) {
                message = `DEADLINE TODAY: "${goal.text}"`;
            } else if (daysLeft === 1) {
                message = `DEADLINE TOMORROW: "${goal.text}"`;
            } else {
                message = `You have ${daysLeft} days left till your deadline: "${goal.text}"`;
            }
...
}, 20000); // Every 20 secondschrome.alarms.create("testNotification", { when: Date.now() + 1000 });
